...
import { ArrowLeft, Brain, Briefcase, Target, DollarSign, Gauge, TrendingUp, Clock, Award, Globe2, HelpCircle, ChevronRight, ChevronDown, AlertCircle, Edit2, Save, X, CheckCircle, ArrowRight } from 'lucide-react';
...
export function Suggestions({ input, onBack }: SuggestionsProps) {
  ...
  const [editableSuggestions, setEditableSuggestions] = useState<GigSuggestion | null>(null);
  ...
  const [editingSection, setEditingSection] = useState<keyof GigSuggestion | null>(null);
  const [editingIndex, setEditingIndex] = useState<number | null>(null);
  const [editValue, setEditValue] = useState('');
  const [showMetadata, setShowMetadata] = useState(false);
  const [metadata, setMetadata] = useState<GigMetadata | null>(null);
  const [editableMetadata, setEditableMetadata] = useState<GigMetadata | null>(null);
    ...
    if (!openai) {
      const fallbackDescriptions: Record<string, JobDescription> = {
        'Sales Representative': {
          ...
          description: 'A Sales Representative is responsible for actively seeking out and engaging with potential customers to generate leads and close sales deals.',
          responsibilities: [
      ...
      setJobDescription(fallbackDescriptions[title] || {
        title,
  ...
  const startEditing = (section: keyof GigSuggestion, index: number, value: string) => {
  ...
  const saveEdit = () => {
    if (editingSection && editingIndex !== null && editableSuggestions) {
      const updatedSuggestions = {
        ...editableSuggestions,
        [editingSection]: editableSuggestions[editingSection].map((item: string, index: number) =>
  ...
  const startEditingSchedule = (scheduleIndex: number, field: string, value: string) => {
  ...
  const saveMetadataEdit = () => {
    ...
    if (editingScheduleIndex !== null) {
      if (editingHourIndex !== null) {
        ...
        updatedMetadata.schedules[editingScheduleIndex].hours[editingHourIndex] = {
          day: day.trim(),
          start,
          end
        };
    ...
    } else if (Array.isArray(updatedMetadata[editingMetadataField as keyof GigMetadata])) {
      // Editing arrays (suggestedNames or industries)
      const arrayField = editingMetadataField as keyof Pick<GigMetadata, 'suggestedNames' | 'industries'>;
      updatedMetadata[arrayField] = editValue.split(',').map(item => item.trim());
    }
  ...
  const addNewHour = (scheduleIndex: number) => {
    ...
    setMetadata(updatedMetadata);
    setEditingScheduleIndex(scheduleIndex);
    setEditingHourIndex(updatedMetadata.schedules[scheduleIndex].hours.length - 1);
  };
  ...
  const removeHour = (scheduleIndex: number, hourIndex: number) => {
    ...
    const updatedMetadata = { ...editableMetadata };
    updatedMetadata.schedules[scheduleIndex].hours = updatedMetadata.schedules[scheduleIndex].hours.filter((_, i) => i !== hourIndex);
        ...
        schedules: [
          {
            ...
            description: 'Traditional work schedule aligned with most business operations',
            hours: [
  ...
  const renderEditableList = (section: keyof GigSuggestion, title: string, icon: React.ReactNode) => {
            ...
            <div className="space-y-8">
              ...
              <div className="bg-white rounded-xl shadow-xl p-6">
                <div className="flex items-center justify-between mb-4">
                  <h2 className="text-xl font-semibold text-gray-900">Suggested Names</h2>
                  <button
                    onClick={() => startEditingMetadata('suggestedNames', metadata.suggestedNames.join(', '))}
              ...
              <div className="bg-white rounded-xl shadow-xl p-6">
                <div className="flex items-center justify-between mb-4">
                  <h2 className="text-xl font-semibold text-gray-900">Relevant Industries</h2>
                  <button
                    onClick={() => startEditingMetadata('industries', metadata.industries.join(', '))}
                    className="text-blue-600 hover:text-blue-700 flex items-center text-sm"
                  >
                    <Edit2 className="w-4 h-4 mr-1" />
                    Edit Industries
              ...
              <div className="bg-white rounded-xl shadow-xl p-6">
                <div className="flex items-center justify-between mb-4">
                  <h2 className="text-xl font-semibold text-gray-900">Recommended Schedules</h2>
                  <button
                ...
                <div className="space-y-6">
                  {metadata.schedules.map((schedule, index) => (
                    <div key={index} className="border border-gray-200 rounded-lg p-4">
                      <div className="flex items-center justify-between mb-4">
                        {editingScheduleIndex === index && editingMetadataField === 'name' ? (
                          <div className="flex items-center space-x-2 flex-1">
                            <input
                              ...
                              className="flex-1 px-3 py-2 border border-blue-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500"
                            />
                            <button onClick={saveMetadataEdit} className="text-green-600 hover:text-green-700">
                              <Save className="w-5 h-5" />
                            </button>
                          ...
                          <h3 className="text-lg font-medium text-gray-900 flex items-center">
                            {schedule.name}
                            <button
                              onClick={() => startEditingSchedule(index, 'name', schedule.name)}
                      ...
                      {editingScheduleIndex === index && editingMetadataField === 'description' ? (
                        ...
                        <div className="flex items-center mb-4">
                          <p className="text-gray-600 flex-1">{schedule.description}</p>
                          <button
                            onClick={() => startEditingSchedule(index, 'description', schedule.description)}
                      ...
                      <div className="space-y-2">
                        {schedule.hours.map((hour, hourIndex) => (
                          <div key={hourIndex} className="flex items-center justify-between bg-gray-50 p-3 rounded group">
                            {editingScheduleIndex === index && editingHourIndex === hourIndex ? (
                              <div className="flex-1 flex items-center space-x-2">
                                <input
                                  type="text"
                              ...
                              <>
                                ...
                                <div className="flex items-center space-x-4">
                                  ...
                                  <div className="hidden group-hover:flex items-center space-x-2">
                                    <button
                                      onClick={() => startEditingHour(index, hourIndex)}
                                      className="text-blue-600 hover:text-blue-700"
                                    >
                                      <Edit2 className="w-4 h-4" />
                                    </button>
                                    <button
                                      onClick={() => removeHour(index, hourIndex)}
                                      className="text-red-600 hover:text-red-700"
                                    >
                                      <X className="w-4 h-4" />
                                    </button>
          ...
          <div className="bg-yellow-50 border-l-4 border-yellow-400 p-4 mb-8">
            <div className="flex items-center">
              ...
              <p className="text-sm text-yellow-700">
                OpenAI API key is missing. AI-powered suggestions are currently using fallback data. 
                Please add your API key to enable real AI suggestions.
              </p>
            </div>
          </div>
        )}
          ...
          <div className="bg-white rounded-xl shadow-xl p-6 mb-8">
            {isSuggestionsLoading ? (
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                <div className="space-y-6">
                  {renderEditableList('jobTitles', 'Suggested Job Titles', <Briefcase className="w-5 h-5 mr-2 text-blue-500" />)}
                  {renderEditableList('deliverables', 'Expected Deliverables', <Target className="w-5 h-5 mr-2 text-green-500" />)}
                  {renderEditableList('kpis', 'Key Performance Indicators', <TrendingUp className="w-5 h-5 mr-2 text-indigo-500" />)}
                </div>

                <div className="space-y-6">
                  {renderEditableList('timeframes', 'Project Timeframes', <Clock className="w-5 h-5 mr-2 text-orange-500" />)}
                  {renderEditableList('requirements', 'Requirements & Certifications', <Award className="w-5 h-5 mr-2 text-red-500" />)}
                  {renderEditableList('compensation', 'Typical Compensation', <DollarSign className="w-5 h-5 mr-2 text-emerald-500" />)}
                  {renderEditableList('languages', 'Language Requirements', <Globe2 className="w-5 h-5 mr-2 text-purple-500" />)}
                </div>

                <div className="lg:col-span-2">
                  {renderEditableList('skills', 'Key Skills Required', <Gauge className="w-5 h-5 mr-2 text-purple-500" />)}
                </div>
              </div>
            )}
          </div>
